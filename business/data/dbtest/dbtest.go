...
func NewUnit(t *testing.T, c *docker.Container, dbName string) (*zap.SugaredLogger, *sqlx.DB, func()) {
	...
	/* We can instantiate one DB for a series of tests and then create little DBs inside that, so each test can run in it's own env.
	In order to avoid collisions in DB names, we use rand.Intn() for constructing a name for the DB(look at the service project).*/
	...
	/* When we construct the log for tests, we're not writing it to stdout, instead we're gonna write it to a buffer.
	With this, we're gonna run those tests and get the logs in a buffer and at the end of the test, we're gonna show the logs.
	With this, we separate the logs from test output.*/
